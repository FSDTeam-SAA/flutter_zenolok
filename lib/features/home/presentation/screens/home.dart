...
class _CalendarHomePageState extends State<CalendarHomePage> {
  ...
  void initState() {
    ...
    // ✅ Events (API)
    // Must be registered in bindings:
    // Get.lazyPut<EventRepo>(() => EventRepoImpl(ApiClient()));
    // Get.lazyPut<EventController>(() => EventController(Get.find<EventRepo>()));
  ...
  /// We use dynamic so this file compiles even if your store is RxMap/Map.
  Iterable<CalendarEvent> _allEventsFromController() {
    final dynamic ec = Get.find<EventController>();

    // Prefer a method if you have it
    try {
      final Iterable<CalendarEvent> all = ec.allEvents();
      return all;
    } catch (_) {}

    // Otherwise read store map values (Map<DateTime, List<CalendarEvent>> or RxMap)
    try {
      final dynamic store = ec.store; // Map or RxMap
      final Iterable<CalendarEvent> all = (store.values as Iterable)
          .expand((v) => (v as List).cast<CalendarEvent>());
      return all;
    } catch (_) {}
  ...
  List<CalendarEvent> _eventsFor(DateTime day) {
    ...
    final List<CalendarEvent> exact =
    Get.find<EventController>().eventsForDay(k);

    // ✅ spanning all-day streaks that cover this day
    // Important: Many APIs store multi-day allDay events only on start day,
    // so we must include them for every day in range.
    final spanning = _allEventsFromController().where((e) =>
    ...
    for (final e in spanning) {
      // avoid duplicates if controller already returns it for the day
  ...
  Future<void> _addEvent(CalendarEvent e) async {
    // create event via API (controller)
    await Get.find<EventController>().createEventFromUi(e);

    // reload month (safe) to reflect server state
    await Get.find<EventController>().loadMonth(_focused.value);

    // keep selection on the created day
    setState(() {
  ...
  Future<void> _openHeaderDatePicker() async {
    ...
    if (result != null) {
      ...
      // ✅ load month for new focused day
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    // ✅ keep UI same: show loading bar in header area
                    Obx(
              ...
              // ✅ Calendar + Event list should rebuild when controller updates
              // Use GetBuilder so UI stays same but refreshes after API loadMonth()/createEvent()
                        ...
                        child: SizedBox(
                          ...
                          child: TableCalendar<CalendarEvent>(
                            ...
                            onPageChanged: (d) {
                              ...
                              Get.find<EventController>().loadMonth(d);
                            },
                            headerVisible: false,
...
//   }
//
//   Iterable<CalendarEvent> _allEvents() {
//     final ec = Get.find<EventController>();
//     return ec.store.values.expand((v) => v);
//   }
//
//
//   List<CalendarEvent> _eventsFor(DateTime day) {
//     final k = _dOnly(day);
//     final exact = Get.find<EventController>().eventsForDay(k);
//
//
...
//                       ),
//                     ),
//                     Obx(() => Get.find<EventController>().loading.value
//                         ? const LinearProgressIndicator(minHeight: 2)
//                         : const SizedBox.shrink()),
//
//                     Stack(
