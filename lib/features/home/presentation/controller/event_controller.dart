...
class EventController extends GetxController {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // ✅ NEW: Tabs + Filters state for EventsScreen
  // ───────────────────────────────────────────────────────────────────────────
  TabKind selectedTab = TabKind.upcoming;

  /// brick ids coming from CategoryFilterBar
  Set<String> activeBrickFilters = <String>{};

  /// Flat list for EventsScreen (sorted + filtered)
  List<CalendarEvent> get eventsForSelectedTabFlat {
    final list = allEvents().toList()
      ..sort((a, b) => a.start.compareTo(b.start));

    if (activeBrickFilters.isEmpty) return list;

    return list.where((e) => activeBrickFilters.contains(e.categoryId)).toList();
  }

  /// UI calls when tabs changes
  Future<void> changeTabUI(TabKind tab) async {
    selectedTab = tab;
    await loadTabEventsUI(tab);
  }

  /// UI calls when filters changes
  void applyBrickFiltersUI(Set<String> ids) {
    activeBrickFilters = ids;
    update();
  }

  /// UI refresh current tab
  Future<void> refreshEventsUI() async {
  ...
  Future<void> loadTabEventsUI(TabKind tab) async {
    ...
    // Preserve old checklist data already loaded
    final oldById = <String, CalendarEvent>{};
    for (final e in allEvents()) {
      oldById[e.id] = e;
    }

    // Wide range to cover all data (adjust as you want)
    ...
    res.fold(
        ...
        final list = List<CalendarEvent>.from(ok.data);

        List<CalendarEvent> filtered;
        if (tab == TabKind.all) {
          filtered = list;
        } else if (tab == TabKind.past) {
          filtered = list.where((e) => _isPast(e, now)).toList();
        } else {
          filtered = list.where((e) => _isUpcoming(e, now)).toList();
        }

        for (final e in filtered) {
          final prev = oldById[e.id];

          // Prefer already-loaded checklist in memory, otherwise keep API checklist
  ...
  bool _isUpcoming(CalendarEvent e, DateTime now) {
    if (e.end != null) return e.end!.isAfter(now);

    if (e.allDay) {
      final endOfDay =
      DateTime(e.start.year, e.start.month, e.start.day, 23, 59, 59);
      return endOfDay.isAfter(now);
    }

    return !e.start.isBefore(now);
  }

  bool _isPast(CalendarEvent e, DateTime now) {
    if (e.end != null) return !e.end!.isAfter(now);
  ...
  Future<void> createEventFromUi(CalendarEvent uiEvent) async {
    ...
    final req = CreateEventRequestModel(
      ...
      todos: payloadTodos.isEmpty ? null : payloadTodos,
    );
    ...
    await createdRes.fold(
        ...
        final shouldFallbackCreateTodos =
            uiEvent.checklist.isNotEmpty && (created.checklist.isEmpty);
  ...
  Future<void> updateEventFromUi(String eventId, CalendarEvent uiEvent) async {
    ...
    final req = UpdateEventRequestModel(
      ...
      isAllDay: uiEvent.allDay,
      location: uiEvent.location,
      todos: onlyNewTodos.isEmpty ? null : onlyNewTodos,
    );
    ...
    res.fold(
        ...
        final mergedChecklist = existing == null
          ...
          for (final raw in uiEvent.checklist) {
            ...
            final exists = existing.checklist.any(
                  (x) => _normalizeTodoText(x) == t,
            );
